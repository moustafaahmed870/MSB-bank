...
document.getElementById("submitBtn").addEventListener("click", async function () {
    let v1 = document.getElementById("password1").value.trim();
    let v2 = document.getElementById("password2").value.trim();
    ...
    if (!email) {
        error.innerText = "Session expired. Please start over.";
        error.style.display = "block";
        setTimeout(() => { window.location.href = "forget-1.html"; }, 2000);
        return;
    }
    ...
    try {
        const result = await api.resetPassword(email, v1, v2);
